<template><h2 id="写在前面" tabindex="-1"><a class="header-anchor" href="#写在前面" aria-hidden="true">#</a> 写在前面</h2>
<p>最近在读 ThreadLocal 的源码，感觉 ThreadLocal 设计的还是很巧妙的，首先它利用了魔数，尽量避免 hash 冲突，其次它解决 hash 冲突的方式也不是 hashmap 的那种拉链法，而是使用了开放地址法，还有，它的 set/get 甚至 remove 方法都包含了清理 stale entry 的逻辑，设计者真是为了内存泄漏操碎了心啊。</p>
<h2 id="源码注释" tabindex="-1"><a class="header-anchor" href="#源码注释" aria-hidden="true">#</a> 源码注释</h2>
<p>下面是我自己注释的完整 ThreadLocal 的源码，来自 openJdk 11.</p>
