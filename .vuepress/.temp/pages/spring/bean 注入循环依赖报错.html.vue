...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><p>后来通过 @lazy 注解，让这个 Bean 惰性初始化解决了这个问题。</p>
...
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br></div></div><p>但是后来转念一想，出现循环依赖的问题，大概率是因为项目的结构不合理，低层组件依赖了高层组件。</p>
<p>一个好的代码结构的依赖关系最好是单向的，比如传统的 mvc 架构，controller -&gt; service -&gt; dao ；再比如 ddd 微服务设计思想也是应用层调用领域层，领域层再调用基础设施。</p>
<p>最后，我修改了项目结构而不是添加 @lazy 解决这个问题。</p>
<hr>
<p>跑个题，聊一下 spring 如何解决 bean 循环依赖的问题</p>
<h2 id="spring-创建-bean-的流程" tabindex="-1"><a class="header-anchor" href="#spring-创建-bean-的流程" aria-hidden="true">#</a> spring 创建 bean 的流程</h2>
<p>实例化，通过反射创建出实例<br>
注入，填充属性<br>
初始化，执行各种回调</p>
<h2 id="三级缓存" tabindex="-1"><a class="header-anchor" href="#三级缓存" aria-hidden="true">#</a> 三级缓存</h2>
<p>singletonObjects 单例池<br>
earlySingletonObjects 半成品池<br>
singletonFactories 工厂方法池，lamda</p>
<h2 id="一级缓存的作用" tabindex="-1"><a class="header-anchor" href="#一级缓存的作用" aria-hidden="true">#</a> 一级缓存的作用</h2>
<p>缓存成品 bean</p>
<h2 id="二级缓存的作用" tabindex="-1"><a class="header-anchor" href="#二级缓存的作用" aria-hidden="true">#</a> 二级缓存的作用</h2>
<p>缓存半成品 bean</p>
<h3 id="三级缓存的作用" tabindex="-1"><a class="header-anchor" href="#三级缓存的作用" aria-hidden="true">#</a> 三级缓存的作用</h3>
<p>缓存能暴露出 bean 的工厂方法，使得可以提前获取到 bean</p>
<h3 id="三级缓存为什么是方法" tabindex="-1"><a class="header-anchor" href="#三级缓存为什么是方法" aria-hidden="true">#</a> 三级缓存为什么是方法</h3>
<p>不仅要暴露出 bean，还要执行一些后置处理器<br>
lamda 存的是这个方法</p>
...
<div class="language-java ext-java line-numbers-mode"><pre v-pre class="language-java"><code><span class="token class-name">Object</span> exposedObject <span class="token operator">=</span> bean<span class="token punctuation">;</span> 
<span class="token keyword">return</span> exposedObject
</code></pre><div class="line-numbers"><span class="line-number">1</span><br><span class="line-number">2</span><br></div></div><p>如果有 aop，就会创建出代理对象再返回</p>
<h3 id="整体流程" tabindex="-1"><a class="header-anchor" href="#整体流程" aria-hidden="true">#</a> 整体流程</h3>
<p>Spring 通过三级缓存解决了循环依赖，其中一级缓存为单例池（singletonObjects）,二级缓存为早期曝光对象earlySingletonObjects，三级缓存为早期曝光对象工厂（singletonFactories）。当 A、B 两个类发生循环引用时，在A 完成实例化后，就使用实例化后的对象去创建一个对象工厂，并添加到三级缓存中，如果 A 被 AOP 代理，那么通过这个工厂获取到的就是 A 代理后的对象，如果 A 没有被 AOP 代理，那么这个工厂获取到的就是 A 实例化的对象。当 A 进行属性注入时，会去创建 B，同时 B 又依赖了 A，所以创建 B 的同时又会去调用 getBean(a) 来获取需要的依赖，此时的getBean(a) 会从缓存中获取，第一步，先获取到三级缓存中的工厂；第二步，调用对象工工厂的 getObject 方法来获取到对应的对象，得到这个对象后将其注入到 B 中。紧接着 B 会走完它的生命周期流程，包括初始化、后置处理器等。当 B 创建完后，会将 B 再注入到 A 中，此时 A 再完成它的整个生命周期。<br>
<img src="https://user-images.githubusercontent.com/43411944/139638865-d9e95af5-fd5f-4106-9468-df69f8e35b54.png" alt="image"></p>
</template>
